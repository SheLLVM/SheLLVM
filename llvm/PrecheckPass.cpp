...
struct Precheck : public FunctionPass {
  ...
  bool runOnFunction(Function &F) override {
      if(!F.hasFnAttribute(Attribute::NoRecurse)) {
          report_fatal_error("Function " + F.getName() + " is not marked with norecurse!");
      }

      if(!F.hasFnAttribute("shellvm-main")) {
          if(F.getUnnamedAddr() != GlobalValue::UnnamedAddr::None) {
              report_fatal_error("Function " + F.getName() + " is not marked as unnamed_addr!");
          }
      } else {
          if(F.getUnnamedAddr() != GlobalValue::UnnamedAddr::Local) {
              report_fatal_error("SheLLVM main function " + F.getName() + " is not marked as local_unnamed_addr!");
          }
      }
