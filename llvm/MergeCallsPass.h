      ...
      if (callers.size() > 1) {
        ...
        for (CallInst *caller : callers) {
          ...
          BasicBlock *returnBlock =
              parentBlock->splitBasicBlock(caller->getNextNode(), "");
        ...
        if (target->arg_size() > 0) {
          ...
          for (Argument &A : target->args()) {
            ...
            // pair.
            PHINode *argNode =
                PHINode::Create(A.getType(), callers.size(), "", callBlock);
            for (CallInst *caller : callers) {
              argNode->addIncoming(caller->getArgOperand(argCtr),
                                   callerToOrigParent[caller]);
            }
        ...
        CallInst *callInstr = CallInst::Create(
        ...
        PHINode *whereFromNode = PHINode::Create(
            Type::getInt32Ty(F.getContext()), callers.size(), "", callInstr);
        // Our default is gonna be a basic block that only contains an
        // 'unreachable' instruction.
        SwitchInst *switchBackInstr =
            SwitchInst::Create(whereFromNode, getUnreachableBlock(F),
                               callerToRet.size(), callBlock);
        int switchCtr = 0;
