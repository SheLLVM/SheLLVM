...
struct Postcheck : public ModulePass {
  ...
  bool runOnModule(Module &M) override {
      // Check 1: Make sure we only have a single function, with the attribute shellvm-main.
      ...
      if(mainFunc && !mainFunc->hasFnAttribute("shellvm-main")) {
          report_fatal_error("SheLLVM main function " + mainFunc->getName() + " has no shellvm-main attribute!");
      }
      ...
      if(mainFunc) {
          ...
          if(!dfsTraverse(cgn)) {
              report_fatal_error("Non-intrinsic external call found within main function!");
          }
      }
